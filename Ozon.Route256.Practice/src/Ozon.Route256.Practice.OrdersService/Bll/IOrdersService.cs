...
public interface IOrdersService
{
    ...
    Task<List<OrdersStatisticEntity>> GetAggregatedOrdersByRegion(RegionEntity[] reqRegions, DateTime sinceTimestamp, CancellationToken token);
    ...
    Task<IReadOnlyCollection<OrderEntity>> GetOrdersByCustomer(int customerId, DateTime sinceTimestamp, PaginationEntity pagination, CancellationToken token);
    Task<OrderStatusEntity> GetOrderStatusAsync(long id, CancellationToken token);
    Task<RegionEntity[]> GetRegions(CancellationToken token);
}